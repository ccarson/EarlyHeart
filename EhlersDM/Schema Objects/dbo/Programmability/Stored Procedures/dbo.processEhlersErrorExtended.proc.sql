CREATE PROCEDURE dbo.processEhlersErrorExtended ( @pErrorTypeID       AS INT
                                                , @pCodeBlockNum      AS INT
                                                , @pCodeBlockDesc     AS SYSNAME
                                                , @pErrorNumber       AS INT
                                                , @pErrorSeverity     AS INT
                                                , @pErrorState        AS INT
                                                , @pErrorProcedure    AS SYSNAME
                                                , @pErrorLine         AS INT
                                                , @pErrorMessage      AS VARCHAR (MAX)
                                                , @pErrorData         AS VARCHAR (MAX) )
AS
/*
************************************************************************************************************************************
...
/**/      , @codeBlockDesc  = @codeBlockDesc01 ; --  Throw application error on doomed transactions ( always a fatal error )
    ...
    IF  XACT_STATE() = -1
    BEGIN
        SELECT  @pErrorTypeID   = 1 ;
        SELECT  @subject        = @serverName + '.' + @databaseName + N' reports a processing error in ' + @pErrorProcedure
              , @body           = @serverName + '.' + @databaseName + N' dbo.processEhlersError cannot log the error because the '
                                ...
                                + @pErrorProcedure + ' needs to execute a ROLLBACK before invoking dbo.processEhlersError.' ;
    END
        ELSE
    BEGIN
...
/**/      , @codeBlockDesc  = @codeBlockDesc02 ; --  or, write the error data error to SQLErrorLog
        ...
        SELECT  @pCodeBlockNum, @pCodeBlockDesc, @pErrorNumber, @pErrorSeverity, @pErrorState, @pErrorProcedure
                    , @pErrorLine, @pErrorMessage, @pErrorData, CONVERT( DATETIME2(7), @errorTime ), @userName ;
...
/**/      , @codeBlockDesc  = @codeBlockDesc03 ; --  format DBMail documenting the error
        ...
        SELECT  @subject = @serverName + '.' + @databaseName + N' reports a processing error in ' + @pErrorProcedure
              , @body    = N'<H1>Database Error on ' + @serverName + '.' + @databaseName + '</H1>'
                         ...
                         + N'<tr><td>Procedure:</td><td></td><td>'          + @pErrorProcedure                          + '</td></tr>'
                         + N'<tr><td>Code Block:</td><td></td><td>'         + CAST( @pCodeBlockNum AS VARCHAR(20) )     + '</td></tr>'
                         + N'<tr><td>Code Description:</td><td></td><td>'   + @pCodeBlockDesc                           + '</td></tr>'
                         + N'<tr><td>Error Line:</td><td></td><td>'         + CAST( @pErrorLine    AS VARCHAR(20) )     + '</td></tr>'
                         + N'<tr><td>Error Code:</td><td></td><td><b>'      + CAST( @pErrorNumber  AS VARCHAR(20) )     + '</b></td></tr>'
                         + N'<tr><td>Error Message:</td><td></td><td><b>'   + @pErrorMessage                            + '</b></td></tr>'
                         + N'<tr></tr>'
                         + N'</TABLE></br>'
                         + N'<H2>All work from ' + @pErrorProcedure + ' has been rolled back.</H2></br>' ;


/**/SELECT  @codeBlockNum   = 04
/**/      , @codeBlockDesc  = @codeBlockDesc04 ; --  attach supporting data from calling procedure if it's available

        IF  ( @pErrorData IS NULL )
            SELECT  @body = @body + N'<H2>' + @pErrorProcedure + N' did not return any supporting error data</H2>' ;
        ELSE
            SELECT  @body = @body + N'<H3>Here is the data from the error: </H3>' + @pErrorData ;
    END
...
/**/      , @codeBlockDesc  = @codeBlockDesc05 ; --  send DBMail to each recipient
    ...
    SELECT  RecipientEMail
     ...
     WHERE  ErrorTypeID = @pErrorTypeID ;
