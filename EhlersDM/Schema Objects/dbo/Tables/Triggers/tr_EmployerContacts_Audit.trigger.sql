CREATE TRIGGER  tr_EmployerContacts_Audit
            ON  dbo.EmployerContacts
AFTER INSERT, UPDATE, DELETE
AS
/*
************************************************************************************************************************************

    Trigger:    tr_EmployerContacts_Audit
     Author:    mkiemen
    Purpose:    writes audit record to dbo.EmployerContactsAudit
    ...
    2)  Write trigger data to dbo.EmployerContactsAudit
...
/**/SELECT  @codeBlockNum = 02, @codeBlockDesc = @codeBlockDesc02 ; --  Write trigger data to dbo.ClientContactsAudit
    IF  @action = 'D'
        INSERT  dbo.EmployerContactsAudit ( EmployerContactsID, EmployerID, ContactID, ChangeType, ModifiedDate, ModifiedUser )
        SELECT  EmployerContactsID, EmployerID, ContactID, @action, GETDATE(), @systemUser
          FROM  deleted ;
    ELSE
        INSERT  dbo.EmployerContactsAudit ( EmployerContactsID, EmployerID, ContactID, ChangeType, ModifiedDate, ModifiedUser )
        SELECT  EmployerContactsID, EmployerID, ContactID, @action, ModifiedDate, ModifiedUser
          FROM  inserted ;
...
BEGIN CATCH
    ...
    SELECT  @errorData = '<b>contents of inserted trigger table</b></br></br>'
                       + '<table border="1">'
                       + '<tr><th>EmployerContactsID</th><th>EmployerID</th><th>ContactID</th>'
                       + '<th>ModifiedDate</th><th>ModifiedUser</th></tr>'
                       + CAST ( ( SELECT  td = EmployerContactsID   , ''
                                        , td = EmployerID           , ''
                                        , td = ContactID            , ''
                                        , td = ModifiedDate         , ''
                                        , td = ModifiedUser         , ''
    ...
    SELECT  @errorData = ISNULL( @errorData, '' )
                       ...
                       + '<table border="1">'
                       + '<tr><th>EmployerContactsID</th><th>ClientID</th><th>ContactID</th>'
                       + '<th>ModifiedDate</th><th>ModifiedUser</th></tr>'
                       + CAST ( ( SELECT  td = EmployerContactsID   , ''
                                        , td = EmployerID           , ''
                                        , td = ContactID            , ''
