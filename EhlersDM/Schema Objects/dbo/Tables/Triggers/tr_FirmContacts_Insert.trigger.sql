...
************************************************************************************************************************************
    ...
    4)  INSERT edata.FirmContacts from trigger data
...
BEGIN TRY
    ...
    DECLARE @codeBlockNum       AS INT
          ...
          , @codeBlockDesc02    AS SYSNAME = 'SELECT current ContactID from edata.FirmContacts for INSERT'
          , @codeBlockDesc03    AS SYSNAME = 'INSERT Conversion.LegacyContacts from trigger tables'
          , @codeBlockDesc04    AS SYSNAME = 'INSERT edata.FirmContacts from trigger data' ;
...
/**/SELECT  @codeBlockNum = 02, @codeBlockDesc = @codeBlockDesc02 ; --  SELECT current ContactID from edata.FirmContacts for INSERT
...
/**/SELECT  @codeBlockNum = 04, @codeBlockDesc = @codeBlockDesc04 ; --  INSERT edata.FirmContacts from trigger data
    INSERT  edata.FirmContacts (
            ContactId, FirmId, FDN, NamePrefix, FirstName, LastName
                , Title, Department, Phone, Email, Fax, CellPhone, Notes
                , ChangeCode, ChangeDate, ChangeBy )
    SELECT  ContactID   = lgc.LegacyContactID
          , FirmID      = ins.FirmID
          , FDN         = frm.Firm + ' [' + frm.City + ',' + frm.State + ']'
...
INNER JOIN  Conversion.LegacyContacts       AS lgc ON lgc.ContactID = lgc.ContactID
...
BEGIN CATCH
    ...
    SELECT  @errorData = '<b>contents of insert to edata.FirmContacts</b></br></br>'
                       + '<table border="1">'
                       + '<tr><th>ContactID</th><th>FirmID</th><th>FDN</th><th>NamePrefix</th><th>FirstName</th><th>LastName</th>'
                       ...
                       + CAST ( ( SELECT  td = lgc.LegacyContactID, ''
                                       ,  td = ins.FirmID, ''
                                       ,  td = frm.Firm + ' [' + frm.City + ',' + frm.State + ']', ''
                                       ,  td = cnv.NamePrefix, ''
                                       ,  td = cnv.FirstName, ''
                                       ,  td = cnv.LastName, ''
                                       ,  td = cnv.Title, ''
