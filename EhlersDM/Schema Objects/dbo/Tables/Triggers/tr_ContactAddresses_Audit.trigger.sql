CREATE TRIGGER  tr_ContactAddresses_Audit
...
************************************************************************************************************************************
    ...
    Trigger:    tr_ContactAddresses_Audit
     Author:    mkiemen
    ...
    2)  INSERT dbo.ContactAddressesAudit from trigger data
...
BEGIN TRY
    ...
    DECLARE @codeBlockNum       AS INT
          ...
          , @codeBlockDesc02    AS SYSNAME  = 'INSERT dbo.ContactAddressesAudit from trigger data' ;
...
/**/SELECT  @codeBlockNum = 02, @codeBlockDesc = @codeBlockDesc02 ; --  INSERT dbo.FirmContactsAudit from trigger data
    IF  @action = 'D'
        INSERT  dbo.ContactAddressesAudit (
                ContactAddressesID, ContactID, AddressID, AddressTypeID, ChangeType, ModifiedDate, ModifiedUser )
        SELECT  ContactAddressesID, ContactID, AddressID, AddressTypeID, @action, GETDATE(), @systemUser
          FROM  deleted ;
    ELSE
        INSERT  dbo.ContactAddressesAudit (
                ContactAddressesID, ContactID, AddressID, AddressTypeID, ChangeType, ModifiedDate, ModifiedUser )
        SELECT  ContactAddressesID, ContactID, AddressID, AddressTypeID, @action, ModifiedDate, ModifiedUser
          FROM  inserted ;
