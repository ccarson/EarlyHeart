CREATE TRIGGER  tr_ClientAddresses_Audit
...
************************************************************************************************************************************
    ...
    Trigger:    tr_ClientAddresses_Audit
     Author:    Chris Carson
    Purpose:    writes audit record to dbo.ClientAddressesAudit
    ...
    2)  INSERT dbo.ClientAddressesAudit from trigger data
...
BEGIN TRY
    ...
    DECLARE @codeBlockNum       AS INT
          ...
          , @codeBlockDesc02    AS SYSNAME  = 'INSERT dbo.ClientAddressesAudit from trigger data' ;
...
/**/SELECT  @codeBlockNum = 02, @codeBlockDesc = @codeBlockDesc02 ; --  INSERT dbo.ClientAddressesAudit from trigger data
    IF  @action = 'D'
        INSERT  dbo.ClientAddressesAudit ( 
                ClientAddressesID, ClientID, AddressID, AddressTypeID, ChangeType, ModifiedDate, ModifiedUser )
        SELECT  ClientAddressesID, ClientID, AddressID, AddressTypeID, 'D', GETDATE(), @systemUser
        FROM    deleted ;
    ELSE
        INSERT  dbo.ClientAddressesAudit ( 
                ClientAddressesID, ClientID, AddressID, AddressTypeID, ChangeType, ModifiedDate, ModifiedUser )
        SELECT  ClientAddressesID, ClientID, AddressID, AddressTypeID, @action, ModifiedDate, ModifiedUser
        FROM    inserted ;
